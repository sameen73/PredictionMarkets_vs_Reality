...
def build_chart(market, probabilities, gas_prices) -> go.Figure:
    fig = go.Figure()
    fig.add_trace(
        go.Scatter(
    ...
    fig.add_hline(
        ...
        line_dash="dash",
        line_color="#dc2626",
        annotation_text=f"${market['threshold']:.2f} threshold",
    ...
    fig.update_layout(
        ...
        yaxis2=dict(title="Gas price", overlaying="y", side="right", tickprefix="$"),
        hovermode="x unified",
...
def main() -> None:
    st.set_page_config(page_title="Gas Prices", page_icon=":fuelpump:", layout="wide")
    st.title("Gas Prices")

    market = select_market()
    probabilities = load_probabilities(market["market_id"])
    gas_prices = load_ground_truth(market["market_id"])

    render_summary(market, probabilities)
    st.plotly_chart(build_chart(market, probabilities, gas_prices), width="stretch")
